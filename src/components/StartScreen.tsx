...
export function StartScreen() {
  ...
  const toggleRememberLastScreen = useCalculatorStore(
    (state) => state.toggleRememberLastScreen
  )
  ...
  return (
    <main className="app start">
      <header className="start-header">
        ...
        <p>
          Välj beräkningssätt först. Då blir inputen tydlig och resultatet kan
          uppdateras direkt.
        </p>
      </header>
      ...
      <section className="mode-list">
        {modes.map((item) => (
          <button
            key={item.mode}
            className="mode-card"
            onClick={() => startCalculation(item.mode)}
          >
      ...
      <label className="remember-row">
        <input
          type="checkbox"
          checked={rememberLastScreen}
          onChange={toggleRememberLastScreen}
        />
